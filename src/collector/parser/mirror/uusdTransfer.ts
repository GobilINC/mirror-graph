...
async function contractTransfer(
  ...
  if (contract.type !== ContractType.PAIR) return

  const positionsRepo = manager.getRepository(AssetPositionsEntity)
  const positions = await assetService().getPositions(
  ...
  const poolPrice =
    positions.uusdPool !== '0' && positions.pool !== '0'
...
async function accountTransfer(
  address: string,
  value: string,
  datetime: Date,
  manager: EntityManager
